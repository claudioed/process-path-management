...
Run: python3 charts/process-path-management/tests/test_service_selectors.py
"""
...
def render(extra_args: list[str]) -> list[dict]:
    out = subprocess.run(
        ["helm", "template", "process-path-management", str(CHART_DIR), *extra_args],
...
def main() -> int:
    ...
    if "process-path-management" not in services:
        failures.append("the OLTP Service was not rendered")
    elif selector_of(services["process-path-management"]).get("app.kubernetes.io/component") != "api":
        failures.append("the OLTP Service selector must pin component=api")

    if "process-path-management-frontend" not in services:
        failures.append("the frontend Service was not rendered with frontend.enabled=true")
    elif selector_of(services["process-path-management-frontend"]).get("app.kubernetes.io/component") != "frontend":
    ...
    fe = services.get("process-path-management-frontend")
