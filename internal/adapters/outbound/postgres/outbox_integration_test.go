...
func TestOutbox_PublishFailure_RollsBackAggregate(t *testing.T) {
	...
	if _, err := uc.Execute(ctx, "PACK", "pack", true, []shared.Capability{"pack"}, shared.DestinationLocationRoleUnset); err == nil {
...
func TestOutboxRelay_PublishesInOrderAndMarksRows(t *testing.T) {
	...
	if _, err := define.Execute(ctx, "SLAM", "slam", true, []shared.Capability{"slam"}, shared.DestinationLocationRoleUnset); err != nil {
		t.Fatalf("define: %v", err)
	}
...
func TestOutboxRelay_SinkFailure_StopsAtFailedRowAndRetriesLater(t *testing.T) {
	...
	for _, id := range []shared.PathId{"A1", "B2", "C3"} {
		if _, err := define.Execute(ctx, id, strings.ToLower(string(id)), true, []shared.Capability{"pick"}, shared.DestinationLocationRoleUnset); err != nil {
